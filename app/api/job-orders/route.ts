...
export async function GET(request: NextRequest) {
  try {
    ...
    const backendResponse = await fetch(`${BACKEND_URL}/api/job-orders?${searchParams.toString()}`, {
      cache: "no-store",
    ...
    console.error(error);
    return Response.json({ success: false, message: "Không thể lấy danh sách", data: null }, { status: 500 });
  }
}
